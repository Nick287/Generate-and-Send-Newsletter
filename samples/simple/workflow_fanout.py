    ...
    async def validate_schema(self, batch: DataBatch, ctx: WorkflowContext[ValidationReport]) -> None:
        ...
        if not request or not request.enable_schema_validation:
            report = ValidationReport(
                ...
                issues_found=0,
                processing_time=0.0,
                details="Schema validation skipped (disabled)",
            )
    ...
    async def validate_quality(self, batch: DataBatch, ctx: WorkflowContext[ValidationReport]) -> None:
        ...
        if not request or not request.enable_quality_validation:
            report = ValidationReport(
                ...
                issues_found=0,
                processing_time=0.0,
                details="Quality validation skipped (disabled)",
            )
            await ctx.send_message(report)
            return
    ...
    async def validate_security(self, batch: DataBatch, ctx: WorkflowContext[ValidationReport]) -> None:
        ...
        if not request or not request.enable_security_validation:
            report = ValidationReport(
                ...
                issues_found=0,
                processing_time=0.0,
                details="Security validation skipped (disabled)",
            )
            await ctx.send_message(report)
            return
        ...
        if request.force_validation_failure:
            issues = max(issues, 4)  # Force security failure

        # Use tiered result like other validators
        result = (
            ValidationResult.VALID
            if issues <= 1
            else (ValidationResult.WARNING if issues <= 2 else ValidationResult.ERROR)
...
async def main():
    """Run the fanout workflow locally with two test cases."""
    logging.basicConfig(level=logging.INFO, format="%(message)s")
    logger = logging.getLogger(__name__)

    wf = create_complex_workflow()
    ...
    svg_file = viz.export(format="svg", filename=os.path.join(_dir, "workflow_fanout.svg"))
    print(f"SVG exported to: {svg_file}\n")

    # ── Test Case 1: Should pass all the way through ──
    # Disable security validation (always produces ERROR) and lower quality
    # threshold so schema+quality warnings don't block the pipeline.
    request_pass = ProcessingRequest(
        data_source="database",
        data_type="analytics",
        processing_priority="high",
        batch_size=500,
        quality_threshold=0.5,          # lowered so warnings pass
        enable_schema_validation=True,
        enable_quality_validation=True,
        enable_security_validation=False,  # always ERROR → disable
        transformations=["normalize", "enrich", "aggregate"],
        description="Test case – should complete successfully",
    )

    print("=" * 60)
    print("TEST 1: Expected to PASS (security off, threshold 0.5)")
    print("=" * 60)
    async for event in wf.run(request_pass, stream=True):
        if event.type == "output":
            print(event.data)

    # ── Test Case 2: Should fail at validation ──
    request_fail = ProcessingRequest(
        data_source="api",
        data_type="customer",
        ...
        description="Test case – should fail at validation",
    )

    print("\n" + "=" * 60)
    print("TEST 2: Expected to FAIL (force_validation_failure=True)")
    print("=" * 60)
    async for event in wf.run(request_fail, stream=True):
        if event.type == "output":
            print(event.data)
