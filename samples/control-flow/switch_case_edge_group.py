...
async def main():
    ...
    svg_file = viz.export(format="svg", filename=os.path.join(_dir, "switch_case_workflow.svg"))
    ...
    for label, email_text in [
        ...
        ("Spam", SPAM_EMAIL),
        ("Ambiguous (Default/Uncertain)", AMBIGUOUS_EMAIL),
    ]:
        print(f"\n--- {label} ---")
        events = await workflow.run(email_text)
