...
CURATE_PROMPT_FILE_V8 = PROMPTS_DIR / "curate-v8.md"
TMP_HTML_FILE = Path("/tmp/openclaw/ai-newsletter-v7.html")
TMP_HTML_FILE_V8 = Path("/tmp/openclaw/ai-newsletter-v8.html")
...
DEFAULT_ACS_SENDER = "DoNotReply@example.com"
...
VALID_TAGS_V8 = {"RESEARCH", "PLATFORM", "INDUSTRY", "TOOLS", "ANALYSIS", "LAUNCH"}
...
def validate_config(doc: Any) -> AppConfig:
    ...
    # Issue number priority: env var > config.yaml > week-based auto-calc
    env_issue = os.environ.get("ISSUE_NUMBER", "").strip()
    if env_issue and env_issue.isdigit() and int(env_issue) >= 1:
        issue_number = int(env_issue)
    elif issue_number is None or issue_number == 1:
        # Auto-calculate from start date (2025-04-28 = issue 1)
        _start = dt.date(2025, 4, 28)
        _weeks = max(0, (dt.date.today() - _start).days) // 7
        issue_number = _weeks + 1
...
_AZURE_UPDATE_PREFIX = re.compile(r"^\[(Launched|In preview|Retired)\]", re.IGNORECASE)


def _is_azure_update(title: str, source: str = "") -> bool:
    """Detect Azure availability announcements that belong in sidebar only."""
    if _AZURE_UPDATE_PREFIX.match(title):
        return True
...
def parse_curated_response(content: str) -> tuple[list[Any], str, str, int]:
    value = (content or "").strip()
    if value.startswith("```"):
        value = re.sub(r"^```[a-zA-Z0-9_-]*\n", "", value)
        value = re.sub(r"\n```$", "", value).strip()
    try:
        ...
        if isinstance(parsed, dict):
            ...
            try:
                hero_image_index = int(parsed.get("hero_image_index", 0))
            except (TypeError, ValueError):
                hero_image_index = 0
            stories = parsed.get("stories", [])
            if isinstance(stories, list):
                return stories, tldr, headline, hero_image_index
    ...
    if obj_start != -1 and obj_end > obj_start:
        try:
            parsed = json.loads(value[obj_start : obj_end + 1])
            ...
            pass
    return parse_json_array(content), "", "", 0
...
def curate_prompt_text(template_version: str = "v7") -> str:
    prompt_file = (
        CURATE_PROMPT_FILE_V8 if template_version == "v8" else CURATE_PROMPT_FILE
    )
...
def sanitize_story(story: dict[str, Any]) -> Optional[dict[str, Any]]:
    ...
    if not title or not link:
        return None
    all_valid_tags = VALID_TAGS | VALID_TAGS_V8
...
def normalize_curated_output(raw_stories: Any) -> list[dict[str, Any]]:
    ...
    # Remove Azure updates from main cards (they belong in sidebar only)
    stories = [
        s
        for s in stories
        if not _is_azure_update(s.get("title", ""), s.get("source", ""))
    ]

    stories.sort(key=lambda s: -s.get("score", 0))
    # Promote major releases to top positions, excluding Azure availability notices
    major = [
        s
        for s in stories
        if _is_major_release(s.get("title", ""))
        and not _is_azure_update(s.get("title", ""), s.get("source", ""))
    ]
    rest = [s for s in stories if s not in major]
    return major + rest
...
def stage_curate(
    ...
    prompt_text = curate_prompt_text(template_version)
    payload = []
    for article in articles:
        payload.append(
            {
    ...
    if template_version == "v8":
        user_prompt = (
            "Curate these articles into the required JSON object "
            "with headline, tldr, hero_image_index, and stories. "
    ...
    try:
        ...
        if template_version == "v8":
            raw_stories, tldr, headline, hero_image_index = parse_curated_response(
                content
            )
            curated = normalize_curated_output(raw_stories)
        else:
    ...
    path.write_text(
        json.dumps(curated_data, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    ...
    return curated, tldr, headline, hero_image_index, critical_failure
...
def _extract_product_names(headline: str) -> list[str]:
    """Extract product/model names from a headline for hero image matching."""
    patterns = [
        r"(?i)(gpt[-\s]?[\d.]+\w*)",
        r"(?i)(claude[-\s]?[\d.]+\w*)",
        r"(?i)(gemini[-\s]?[\d.]+\w*)",
...
def _find_best_hero_index(
    headline: str, stories: list[dict[str, Any]], current_index: int
) -> int:
    """If headline mentions a specific product and the current hero story doesn't
    match, find a story that does."""
    if not stories or not headline:
        return current_index
    product_names = _extract_product_names(headline)
    if not product_names:
        return current_index
    hero_title = (
        stories[current_index].get("title", "").lower()
        if current_index < len(stories)
        else ""
    )
    for name in product_names:
        if name in hero_title:
            return current_index
    for idx, story in enumerate(stories):
        title = story.get("title", "").lower()
        for name in product_names:
            if name in title and get_image_or_placeholder(story):
                return idx
    return current_index
...
def load_curated_stories(path: Path) -> tuple[list[dict[str, Any]], str, str, int]:
    raw = json.loads(path.read_text(encoding="utf-8"))
    if isinstance(raw, dict):
        tldr = str(raw.get("tldr", "")).strip()
        headline = str(raw.get("headline", "")).strip()
        try:
            hero_image_index = int(raw.get("hero_image_index", 0))
        except (TypeError, ValueError):
            hero_image_index = 0
        stories = raw.get("stories", [])
        if isinstance(stories, list):
            return normalize_curated_output(stories), tldr, headline, hero_image_index
    if not isinstance(raw, list):
        raise ValueError("Expected curated file to contain a JSON array or object")
    return normalize_curated_output(raw), "", "", 0
...
def format_short_date(published_date: Optional[str]) -> str:
    if not published_date:
        return ""
    try:
        parsed = dt.datetime.fromisoformat(
...
def detect_azure_badge(title: str) -> tuple[str, str]:
    lower = title.lower()
    if "preview" in lower or "public preview" in lower:
        return "Preview", "#F39C12"
    return "GA", "#27AE60"
...
def extract_azure_sidebar_items(
    ...
    for article in azure_articles:
        if article.link in seen_links:
            continue
        seen_links.add(article.link)
        badge, badge_color = detect_azure_badge(article.title)
        items.append(
            {
                "title": truncate_text(article.title, 80),
                "link": article.link,
                "date": format_sidebar_date(article.published_date),
                "short_date": format_short_date(article.published_date),
                "badge": badge,
...
def compose_html(
    ...
    tldr: str = "",
    headline: str = "",
    hero_image_index: int = 0,
) -> str:
    if template_version == "v8":
        return compose_html_v8(
            stories,
            scanned_articles,
            config,
            date_range,
    ...
    return compose_html_v7(stories, scanned_articles, config, date_range)
...
def compose_html_v8(
    ...
    template_path = TEMPLATES_DIR / "v8.html"
    template = template_path.read_text(encoding="utf-8")
    ...
    result = template.replace("{{ISSUE_NUMBER}}", str(config.issue_number))
    result = result.replace("{{ISSUE_DATE}}", issue_date)
    ...
    result = result.replace("{{TLDR}}", escape_html(tldr))

    # Hero title: editorial headline from LLM, fallback to first story title
    hero_title = (
        headline if headline else (stories[0].get("title", "") if stories else "")
    )
    result = result.replace("{{HERO_TITLE}}", escape_html(hero_title))

    # Hero image: pick from hero_image_index story, with headline-match sanity check
    hero_idx = _find_best_hero_index(headline, stories, hero_image_index)
    if hero_idx < 0 or hero_idx >= len(stories):
        hero_idx = 0
    hero_img_story = stories[hero_idx] if stories else {}
    hero_image_url = get_image_or_placeholder(hero_img_story)
    if not hero_image_url and stories:
        for s in stories:
            candidate = get_image_or_placeholder(s)
            if candidate:
                hero_img_story = s
                hero_image_url = candidate
                break

    result = result.replace("{{HERO_IMAGE}}", escape_html(hero_image_url or ""))
    result = result.replace(
        "{{HERO_LINK}}", escape_html(hero_img_story.get("link", "#"))
    )

    # Image caption: selected article's title + source + date
    result = result.replace(
        "{{HERO_IMG_TITLE}}", escape_html(hero_img_story.get("title", ""))
    )
    result = result.replace(
        "{{HERO_IMG_SOURCE}}", escape_html(hero_img_story.get("source", ""))
    )
    result = result.replace(
        "{{HERO_IMG_DATE}}",
        escape_html(format_short_date(hero_img_story.get("published_date"))),
    )

    # Legacy placeholders (kept for backward compat with any external tooling)
    result = result.replace(
        "{{HERO_SOURCE}}", escape_html(hero_img_story.get("source", ""))
    )
    result = result.replace(
        "{{HERO_DATE}}",
        escape_html(format_short_date(hero_img_story.get("published_date"))),
    ...
    for i in range(6):
        ...
        if i < len(stories):
            story = stories[i]
            tag = str(story.get("tag", "")).upper()
            tag_color = TAG_COLORS_V8.get(tag, "#6B7280")
            tag_display = story.get("tag", "")
            result = result.replace(
                "{{%s_TITLE}}" % prefix, escape_html(story.get("title", ""))
            )
            result = result.replace(
            ...
            result = result.replace("{{%s_TAG}}" % prefix, escape_html(tag_display))
            result = result.replace("{{%s_TAG_COLOR}}" % prefix, tag_color)
            result = result.replace(
                "{{%s_DATE}}" % prefix,
                escape_html(format_short_date(story.get("published_date"))),
            )
            result = result.replace(
                "{{%s_TIME}}" % prefix, str(story.get("read_time_minutes", 3))
            )
    ...
    sidebar_items = extract_azure_sidebar_items(scanned_articles, max_items=6)
    for i in range(6):
        prefix = "AZ%d" % (i + 1)
        if i < len(sidebar_items):
            item = sidebar_items[i]
            ...
            result = result.replace("{{%s_LINK}}" % prefix, escape_html(item["link"]))
            result = result.replace(
                "{{%s_DATE}}" % prefix, escape_html(item["short_date"])
            )
            result = result.replace("{{%s_BADGE}}" % prefix, item["badge"])
            result = result.replace("{{%s_BADGE_COLOR}}" % prefix, item["badge_color"])
        else:
            result = result.replace("{{%s_TITLE}}" % prefix, "")
            result = result.replace("{{%s_LINK}}" % prefix, "#")
            result = result.replace("{{%s_DATE}}" % prefix, "")
            result = result.replace("{{%s_BADGE}}" % prefix, "")
            result = result.replace("{{%s_BADGE_COLOR}}" % prefix, "#27AE60")
...
def write_html_outputs(
    ...
    tmp_file = TMP_HTML_FILE_V8 if template_version == "v8" else TMP_HTML_FILE
...
def success_summary_message(
    ...
    return (
        "✅ AI Weekly Digest ready: %s articles, %s stories, %s images, "
        "top: %s, send: %s"
...
def full_pipeline(
    ...
    template_version = getattr(args, "template", "v8")
    curated, tldr, headline, hero_image_index, llm_failed = stage_curate(
        enriched, config, date_label, logger, template_version=template_version
    )
    ...
    write_html_outputs(date_label, html_body, logger, template_version=template_version)
...
def compose_only(
    ...
    curated, tldr, headline, hero_image_index = load_curated_stories(curated_file)
    articles: list[Article] = []
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["v7", "v8"],
        default="v8",
        help="Template version: v7 (legacy) or v8 (Bloomberg-style, default)",
    )
    return parser.parse_args()


def load_runtime_config(
    logger: logging.Logger, template_version: str = "v8"
) -> tuple[AppConfig, list[FeedSource]]:
...
def main() -> int:
    ...
    try:
        config, feeds = load_runtime_config(logger, template_version=args.template)
        if args.legacy:
        ...
        if args.compose_only:
            return compose_only(config, logger, template_version=args.template)
